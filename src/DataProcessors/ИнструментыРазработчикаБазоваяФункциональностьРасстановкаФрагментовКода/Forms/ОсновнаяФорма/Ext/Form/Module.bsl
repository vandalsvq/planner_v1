...
Функция ИзменитьТекстыМодулей(ВыборПользователя, Сценарий)
	...
	СтандартныеПодсистемыКлиентСервер.НовыйРезультатВыполнения(Сценарий);
	ВыводПредупреждения = Сценарий.ВыводПредупреждения;
	ВыводПредупреждения.Использование = Истина;
	ВыводПредупреждения.Текст =
...
Функция МестаИспользованияМетода(ТекстМодуля, ПолноеИмяМетода)
	...
	Пока Истина Цикл
		Позиция = Найти(Остаток, ПолноеИмяМетода);
		Если Позиция = 0 Тогда
			Прервать;
		КонецЕсли;
