...
Функция ТаблицаДанныхЗаголовкаПакета() Экспорт
	...
	Если ТипЗнч(ПолеТаблицаДанныхЗаголовкаПакета) <> Тип("ТаблицаЗначений") Тогда
		...
		одОписаниеТипаСтрока = одОписаниеТипа("Строка");
		одОписаниеТипаБулево = одОписаниеТипа("Булево");
		
		Колонки.Добавить("ТипОбъектаСтрокой",            одОписаниеТипаСтрока);
		Колонки.Добавить("КоличествоОбъектовВИсточнике", одОписаниеТипа("Число"));
		Колонки.Добавить("ПоляПоиска",                   одОписаниеТипаСтрока);
		Колонки.Добавить("ПоляТаблицы",                  одОписаниеТипаСтрока);
		
		Колонки.Добавить("ТипИсточникаСтрокой", одОписаниеТипаСтрока);
		Колонки.Добавить("ТипПриемникаСтрокой", одОписаниеТипаСтрока);
		
		Колонки.Добавить("СинхронизироватьПоИдентификатору",    одОписаниеТипаБулево);
		Колонки.Добавить("ЭтоУдалениеОбъекта",                  одОписаниеТипаБулево);
		Колонки.Добавить("ЭтоКлассификатор",                    одОписаниеТипаБулево);
		Колонки.Добавить("ИспользоватьПредварительныйПросмотр", одОписаниеТипаБулево);
	КонецЕсли;
...
//         * СобиратьСтатистикуКлассификаторов - Булево - Флаг того, что данные по классификаторам будут включены в статистику.
//                                                        Под классификаторами понимаются справочники, ПВХ, планы счетов, ПВР,
//                                                        у которых в ПКО установлены СинхронизироватьПоИдентификатору 
//                                                        и ПродолжитьПоискПоПолямПоискаЕслиПоИдентификаторуНеНашли
// 
Процедура ВыполнитьАнализСообщенияОбмена(ПараметрыАнализа = Неопределено) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ТаблицаДанныхЗаголовкаПакета().Индексы.Добавить("ТипИсточникаСтрокой, ТипПриемникаСтрокой");
			
			Для Каждого СтрокаТаблицы Из ТаблицаДанныхЗаголовкаПакета() Цикл
				
				Отбор = Новый Структура("ТипИсточникаСтрокой, ТипПриемникаСтрокой", СтрокаТаблицы.ТипИсточникаСтрокой, СтрокаТаблицы.ТипПриемникаСтрокой);
		...
		Если ИмяУзла = "Объект" Тогда
			...
			Если ТипОбъектаСтрокой <> "КонстантыНабор" Тогда
				...
				ИначеЕсли Не (ПКО.СинхронизироватьПоИдентификатору И ПКО.ПродолжитьПоискПоПолямПоискаЕслиПоИдентификаторуНеНашли) И ПКО.СинхронизироватьПоИдентификатору Тогда
					...
					// не отображаются пользователю в таблице информации статистики. Поэтому сбор информации статистики для них не нужен. 
