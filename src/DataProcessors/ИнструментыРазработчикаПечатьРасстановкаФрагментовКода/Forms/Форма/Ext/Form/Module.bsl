...
Процедура НайтиФормыИспользующиеГруппуПечать()
	...
	Для Каждого ОписаниеФормы Из СписокВсехФормКонфигурации Цикл
		...
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Анализ форм... (%1 из %2)'"),
			Счетчик, СписокВсехФормКонфигурации.Количество());
...
Процедура ПроверитьНеобходимостьВставкиКодаВМодулях()
	...
	Для Каждого Модуль Из ОбрабатываемыеМодули Цикл
		...
		Если Найти(ТекстМодуля.ПолучитьТекст(), Модуль.ТекстДляВставки) Тогда
...
Процедура ВставитьКодВПроцедуруМодуля(ТекстМодуля, ИмяПроцедуры, ШаблонПроцедуры, Знач ТекстДляВставки, Знач ТекстДляПроверки = "")
	...
	Если Найти(ТекстМодуля.ПолучитьТекст(), ТекстДляПоиска) > 0 Тогда
		Для НомерСтроки = 1 По ТекстМодуля.КоличествоСтрок() Цикл
			Строка = СокрЛ(ТекстМодуля.ПолучитьСтроку(НомерСтроки));
			Если Лев(Строка, СтрДлина(ТекстДляПоиска)) = ТекстДляПоиска Тогда
	...
	Если Не ПустаяСтрока(ТекстДляПроверки) Тогда
		Если Найти(ТекстПроцедуры.ПолучитьТекст(), ТекстДляПроверки) > 0 Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПроцедураНайдена Тогда
		ТекстДляВставки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонПроцедуры,
			ТекстДляВставки);
		...
		Для НомерСтроки = 1 По ТекстМодуля.КоличествоСтрок() Цикл
			...
			Если Лев(Строка, 5) = "#Если" Или Лев(Строка,5) = "Перем" Тогда
				ВозможноеМестоВставки = НомерСтроки + 1;
			ИначеЕсли Лев(Строка, 1) = "&" Или Лев(Строка, 9) = "Процедура" Или Лев(Строка, 7) = "Функция" Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ВставитьПроцедуруВМодуль(ТекстМодуля, ИмяПроцедуры, Знач ТекстДляВставки)
	...
	Для НомерСтроки = -ТекстМодуля.КоличествоСтрок() По -1 Цикл
		Строка = СокрЛП(ТекстМодуля.ПолучитьСтроку(-НомерСтроки));
		Если Лев(Строка, 14) = "КонецПроцедуры" Или Лев(Строка, 12) = "КонецФункции" Или Лев(Строка,5) = "Перем" Тогда
			ВозможноеМестоВставки = -НомерСтроки + 1;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Функция ПутьКФайлуМодуляМенеджера(ПолноеИмяОбъектаМетаданных)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонИмениФайла,
		ИмяПапки, ПолноеИмяОбъектаМетаданных);
...
Функция ПутьКФайлуМодуляФормы(ПолноеИмяФормы)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонИмениФайла, ИмяПапки, ПолноеИмяФормы)
...
Функция ПутьКФайлуОбщегоМодуля(ПолноеИмяМодуля)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонИмениФайла, ИмяПапки, ПолноеИмяМодуля)
	...
	Для Каждого ОписаниеОбъектаМетаданных Из Коллекция.ПолучитьЭлементы() Цикл
		Если ОписаниеОбъектаМетаданных.ЭтоКоманда Тогда
			...
			Для Каждого Тип Из ОписаниеОбъектаМетаданных.ТипПараметраКоманды.Типы() Цикл
				...
				СписокРолейДляПроверки = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(СписокРолейДляПроверки(Команда, ОбъектМетаданных));
				...
				Если Не ПустаяСтрока(СтрокаУсловий) Тогда
					КодДляРазмещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомандыСОграничениемВидимостиПоУсловию(),
						СтрокаУсловий, СтрЗаменить(КодДляРазмещения, Символы.ПС, Символы.ПС + Символы.Таб));
				КонецЕсли;
...
Процедура ЗагрузитьКоллекциюВДеревоОбъектовМетаданных(ПолноеИмяКоллекцииМетаданных, ДеревоОбъектов)
	...
	ЧастиИмени = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПолноеИмяКоллекцииМетаданных, ".", Истина);
	...
	Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
		Если ЭтоОбщаяКоманда(ОбъектМетаданных) Тогда
			Если ЭтоКомандаПечати(ОбъектМетаданных) Тогда
				...
				ОписаниеОбъектаМетаданных.ПутьКФайлу = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонИмениФайла,
					ИмяПапки,
		...
		Иначе
			...
			Если КомандыПечати.Количество() > 0 Тогда
				...
				Для Каждого ОписаниеКоманды Из ОписаниеОбъектаМетаданных.Строки Цикл
					...
					ОписаниеКоманды.ПутьКФайлу = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонИмениФайла,
						ИмяПапки,
			...
			Если Параметр.Ключ = "ДополнительныеПараметры" Тогда
				Для Каждого ДополнительныйПараметр Из Параметр.Значение Цикл
					Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ШаблонДополнительногоПараметра, ДополнительныйПараметр.Ключ, ДополнительныйПараметр.Значение);
				КонецЦикла;
			Иначе
				Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонПараметра, Параметр.Ключ, Параметр.Значение);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ДобавитьОсновныеПараметрыКоманды(КомандаПечати, Знач Строка, МестоположениеИсходногоКода)
	...
	Позиция = Найти(Строка, ТекстДляПоиска);
...
Процедура ДобавитьОпциюПроверкиПроведения(КомандаПечати, Знач ИсходнаяСтрока)
	...
	Позиция = Найти(ИсходнаяСтрока, ТекстДляПоиска);
	Если Позиция > 0 Тогда
