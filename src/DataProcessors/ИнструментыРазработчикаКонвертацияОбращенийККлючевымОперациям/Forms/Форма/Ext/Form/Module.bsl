...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ПозицияСервераНачало = Найти(СтрокаСоединения, "Srvr=") + 6;
		СтрокаСоединения = Сред(СтрокаСоединения, ПозицияСервераНачало);
		ПозицияСервераКонец = Найти(СтрокаСоединения, ";");
		ИмяСервера = Лев(СтрокаСоединения, ПозицияСервераКонец - 2);
		
		ПозицияИБНачало = Найти(СтрокаСоединения, "Ref=") + 5;
		СтрокаСоединения = Сред(СтрокаСоединения, ПозицияИБНачало);
		ПозицияИБКонец = Найти(СтрокаСоединения, ";");
...
Функция ПреобразоватьОбращенияККлючевымОперациям()
	...
	Для Каждого ИмяПодсистемы Из ПодсистемыДляКонвертации Цикл
		ИерархияПодсистемы = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИмяПодсистемы, ".");
...
Функция НайтиКонецФрагмента(Знач СтрокаТекста, Знач НачалоФрагмента)
	...
	Если Найти(СтрокаТекста, ПервыйВариант) = 0 И Найти(СтрокаТекста, ВторойВариант) = 0
		И Найти(СтрокаТекста, ТретийВариант) = 0 И Найти(СтрокаТекста, ЧетвертыйВариант) = 0 Тогда
		Возврат 0;
	КонецЕсли;	
...
&НаСервере
// Функция заменяет фрагмент строки на переданный
//
// Параметры:
