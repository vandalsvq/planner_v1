...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДобавитьТипыОбъектовМетаданных("ДополнительныеПечатныеФормыОбъекта");
...
Процедура ДобавитьТипыОбъектовМетаданных(ИмяКоманды)
	...
	Для Каждого Тип Из МассивТипов Цикл
		...
		Если Найденные.Количество() = 0 Тогда
			...
			СтрокаОбъектаМетаданных.Вид           = Лев(СтрокаОбъектаМетаданных.ПолноеИмя, Найти(СтрокаОбъектаМетаданных.ПолноеИмя, ".") - 1);
...
Функция ОпределитьОшибкиВнедренияКоторыеМожноИсправить()
	...
	Для Каждого СтрокаОбъектаМетаданных Из ОбъектыМетаданных Цикл
		...
		Если ЗначениеЗаполнено(СтрокаОбъектаМетаданных.ФормаОбъектаПолноеИмя) Тогда
			...
			Если ФайлМодуля.Существует() Тогда
				...
				Если Найти(Содержимое, ПоискКода.ПриСозданииНаСервере) <> 0 Тогда
					Если СтрокаОбъектаМетаданных.ЗаполнениеОбъекта Тогда
						Если Найти(Содержимое, ПоискКода.ВыполнитьКоманду) <> 0 И Найти(Содержимое, ПоискКода.ВыполнитьКомандуСервер) <> 0 Тогда
	...
	Если ОбщийТекстОшибок <> "" Тогда
		РезультатВыполнения = Новый Структура("ВыводПредупреждения",
			Новый Структура("Использование, Текст, ТекстОшибок, ПутьКРеквизитуФормы",
				Истина,
	...
	Для Каждого СтрокаОбъектаМетаданных Из Найденные Цикл
		...
		Если НЕ СтрокаОбъектаМетаданных.ФормаОбъектаПодключена И НЕ СтрокаОбъектаМетаданных.ФормаОбъектаЕстьКритичныеОшибки Тогда
			...
			Если ПодключитьКодПриСозданииНаСервере Тогда
				ПозицияПроцедураПриСозданииНаСервере = Найти(ТекстМодуля, КодПроцедураПриСозданииНаСервере);
				Если ПозицияПроцедураПриСозданииНаСервере = 0 Тогда
					...
					ПозицияНачалаОбработчикиСобытий = Найти(ТекстМодуля, КодОбработчикиСобытийФормы);
				...
				Иначе
					...
					ПозицияКонецПроцедуры = ПозицияПроцедураПриСозданииНаСервере + Найти(Остаток, КодКонецПроцедуры) - 1;
			...
			Если ПодключитьКодВыполнитьКоманду Тогда
				ПозицияОбработчикиКомандФормы = Найти(ТекстМодуля, КодОбработчикиКомандФормы);
				Если ПозицияОбработчикиКомандФормы = 0 Тогда
					ПозицияСлужебныеПроцедурыИФункции = Найти(ТекстМодуля, КодСлужебныеПроцедурыИФункции);
			...
			Пока Найти(ТекстМодуля, Символы.ПС + Символы.ПС + Символы.ПС) > 0 Цикл
				ТекстМодуля = СтрЗаменить(ТекстМодуля, Символы.ПС + Символы.ПС + Символы.ПС, Символы.ПС + Символы.ПС);
			КонецЦикла;
			
			Если Прав(ТекстМодуля, 1) <> Символы.ПС Тогда
