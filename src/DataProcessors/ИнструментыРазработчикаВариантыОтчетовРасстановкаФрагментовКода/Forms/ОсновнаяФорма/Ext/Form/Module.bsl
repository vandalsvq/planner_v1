...
Процедура ВыгрузкаМодулейКаталогПриИзменении(Элемент)
	...
	Если Прав(ВыгрузкаМодулейКаталог, 1) <> Слеш Тогда
...
Процедура ПроанализироватьНастройкиРазработчикаСервер()
	...
	ДеревоВариантов = ВариантыОтчетов.ДеревоНастроекВариантовОтчетовКонфигурации();
		...
		Если Не СтрокаОтчет.ОпределитьНастройкиФормы Тогда
			Попытка
				...
				Если ОтчетОбъект.ВерсияСтандартныхФункцийОтчетов() = "1" Тогда
					...
					НастройкиФормы = ФункцииОтчетовКлиентСервер.ПолучитьНастройкиОтчетаПоУмолчанию();
...
Процедура НастройкиАдминистратораДобавить(СтрокаТаблицы, Тип, Значение, ПредставлениеЗначения);
	...
	КонецЕсли;
	ДирективаПредставление = Директива.Тип
		+ ": " + ПредставлениеЗначения + ".";
	//ДирективаПредставление = Директива.ИмяОтчета
	//	+ " / " + Директива.КлючВарианта
	//	+ " / " + Директива.Тип
	//	+ ": " + ПредставлениеЗначения + ".";
...
Функция ИзменитьТекстыМодулей(ВыборПользователя, Сценарий)
	...
	//           "Удалить весь код настройки отчета <ИмяОтчета> из ВариантыОтчетовПереопределяемый.НастроитьВариантыОтчетов(),
	//           кроме вызова ВариантыОтчетов.НастроитьОтчетВМодулеМенеджера"
	...
	Для Каждого СтрокаОтчетИзФормы Из НастройкиОтчетов Цикл
		...
		Если Не ПустаяСтрока(НастроитьВариантыОтчета_БывшееТело) Тогда
			Если Найти(НастроитьВариантыОтчета_БывшееТело, СтрокаРежимВыводаИстина) > 0
				Или Найти(НастроитьВариантыОтчета_БывшееТело, "НастройкиОтчета.ГруппироватьПоОтчету = Истина;") > 0 Тогда
				ГруппироватьПоОтчету = Истина;
			ИначеЕсли Найти(НастроитьВариантыОтчета_БывшееТело, СтрокаРежимВыводаЛожь) > 0
				Или Найти(НастроитьВариантыОтчета_БывшееТело, "НастройкиОтчета.ГруппироватьПоОтчету = Ложь;") > 0 Тогда
		...
		Для Каждого СтрокаВариантИзФормы Из Найденные Цикл
			...
			Для Каждого КлючИЗначение Из НастройкиВарианта.Размещение Цикл
				...
				НастроитьВариантыОтчета_Тело = (
					НастроитьВариантыОтчета_Тело
					+ СимволыПСТаб + СтрЗаменить(Код, "%1", "Метаданные." + СтрЗаменить(ПодсистемаПолноеИмя, "Подсистема.", "Подсистемы."))
				);
			КонецЦикла;
		КонецЦикла;
		...
		Если СтрокаОтчетИзФормы.ДобавитьНастройкиФормы Тогда
			...
			Настройки_Умолчания = ФункцииОтчетовКлиентСервер.ПолучитьНастройкиОтчетаПоУмолчанию();
	...
	Если ЕстьИзмененияДляОбщегоМодуля Тогда
		...
		Для Каждого СтрокаОтчетИзФормы Из НастройкиОтчетов Цикл
			Если Лев(СтрокаОтчетИзФормы.БиблиотекаПолноеИмя, 32) = "Подсистема.СтандартныеПодсистемы" Тогда
				Продолжить;
			КонецЕсли;
		...
		Позиция1 = Найти(ПереопределяемыйМодульТекст, "Процедура НастроитьВариантыОтчетов(Настройки) Экспорт");
		Остаток = Сред(ПереопределяемыйМодульТекст, Позиция1 + СтрДлина("Процедура НастроитьВариантыОтчетов(Настройки) Экспорт"));
		Позиция2 = Найти(Остаток, "КонецПроцедуры");
...
Функция ТелоПроцедуры(ТекстМодуля, УсловияПоиска)
	...
	Для НомерУровня = 1 По УсловияПоиска.УровниКоличество Цикл
		...
		Позиция = Найти(ТекстМодуля, УровеньПоиска.Объявление);
		...
		Если НомерУровня = 1 Тогда // Сама процедура.
			...
			ПозицияЗавершения = Найти(Остаток, УровеньПоиска.Завершение);
...
Процедура УдалитьПроцедуруИлиФункцию(ТекстМодуля, Объявление, Завершение)
	ПозицияНачала = Найти(ТекстМодуля, Объявление);
	Если ПозицияНачала = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Остаток = Сред(ТекстМодуля, ПозицияНачала);
	ПозицияОкончания = Найти(Остаток, Завершение);
...
Функция ЗначениеВКод(Значение)
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		Возврат КодМногострочногоТекста(Значение, Найти(Значение, " ") > 0);
