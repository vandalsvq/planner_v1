...
&НаКлиенте
Процедура ОткрытьФормаДобавленияПолучателей(ЗакрыватьПриВыборе)
    
    Перем ВыбранныеПользователи, ПараметрыФормыВыбора, Строка;
    ...
    ПараметрыФормыВыбора.Вставить("ЗакрыватьПриВыборе", ЗакрыватьПриВыборе);
...
Процедура ДобавитьПеретащитьПолучателя(ПолучательИлиНаборПолучателей)
	...
	Если ЭтоПодборПользователейИлиГрупп() Тогда
...
&НаКлиенте
Функция ЭтоПодборПользователейИлиГрупп()
	Возврат ТипПолучателейРассылки.СодержитТип(Тип("СправочникСсылка.Пользователи"));
КонецФункции
