...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПриСозданииФормыНаСервере(ЭтотОбъект, СтандартнаяОбработка, Отказ) Тогда
		Возврат;
	КонецЕсли;
...
Процедура СтрокаПоискаПриИзменении(Элемент)
	...
	Если СтрокаПоиска <> "" Тогда
		...
		ПроверитьИндексППД();
		ЭтотОбъект.ТекущийЭлемент = ?(НЕ РасширенныйПоиск, Элементы.СтрокаПоиска, Элементы.Список);
...
Процедура УстановитьЗаголовокВидаПанелиНавигации(ЗначениеОтбора = Неопределено)
	...
	Иначе
		...
		Элементы.ВыборВариантаНавигации.Подсказка = НСтр("ru = 'Выбор варианта навигации'");
	...
	Если Настройки.ИмяДерева <> "Категории" Тогда
		...
		Если ЗначениеНастроек.РазвернутыеУзлы.Количество() Тогда
			ОпределитьИдентификаторыРазвернутыхУзлов(ЗначениеНастроек.РазвернутыеУзлы, 
				СоответсвиеИдентификаторовРазвернутыхУзлов, ЭтотОбъект[Настройки.ИмяДерева].ПолучитьЭлементы());
		КонецЕсли;
...
Процедура ПроверитьИндексППД()
	...
	Если Не ИндексППДАктуален И ИнформационнаяБазаФайловая Тогда
		...
		ОбработчикОповещенияОЗакрытии = Новый ОписаниеОповещения("ВопросООбновленииПолнотекстовогоПоискаПослеЗавершения", ЭтотОбъект);
		ПоказатьВопрос(ОбработчикОповещенияОЗакрытии, НСтр("ru='Индекс полнотекстового поиска неактуален. Обновить индекс?'"), РежимДиалогаВопрос.ДаНет);
...
&НаКлиенте
Процедура ВопросООбновленииПолнотекстовогоПоискаПослеЗавершения(РезультатВопроса, ДополнительныеПараметры) Экспорт
